...
def sma_crossover_signal(df, window_list):
    ...
    for w in window_list:
        sma_crossover_['sma_cross_sig_{}'.format(w)] = np.floor(.5*(np.sign(close - close.rolling(w).mean())+1))
...
def rsi_signal(df, window_list):
    ...
    for w in window_list:
        ...
        rsi_signal_['rsi_sig_{}'.format(w)] = rsi
    return rsi_signal_
...
def fi_signal(df, window_list):
    ...
    for w in window_list:
        ...
        long = s>=0
        s = pd.Series(np.where(long,1,0), index=s.index)
        signal_['force_index_sig_{}'.format(w)] = s
    return signal_
